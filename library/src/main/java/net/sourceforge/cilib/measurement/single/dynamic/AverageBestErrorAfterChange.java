...
/**
 * AverageBestErrorAfterChange computes the average of the differences between
 ...
 * fitness at the beginning of each cycle averages them.
 ...
 * the measurement
 */
public class AverageBestErrorAfterChange extends DynamicMeasurement<Real> {
    ...
    public AverageBestErrorAfterChange() {
        super();
    }

    public AverageBestErrorAfterChange(AverageBestErrorAfterChange copy) {
    ...
    @Override
    public AverageBestErrorAfterChange getClone() {
        return new AverageBestErrorAfterChange(this);
    }

    @Override
    public synchronized Real getValue(Algorithm algorithm) {
        if ((algorithm.getIterations() - 1) % cycleSize == 0 && algorithm.getIterations() != 1) {
