...
/**
 * Implementation of the IPSOLT velocity update equation, which also uses
 * the inertia weight on the cognitive component.
 */
public final class IPSOLTVelocityProvider implements VelocityProvider {
    ...
    public IPSOLTVelocityProvider() {
        this(ConstantControlParameter.of(0.729844),
            ConstantControlParameter.of(1.496180),
            ConstantControlParameter.of(1.496180));
    }

    public IPSOLTVelocityProvider(ControlParameter inertia, ControlParameter social, ControlParameter cog) {
     ...
     */
    public IPSOLTVelocityProvider(IPSOLTVelocityProvider copy) {
     ...
     */
    @Override
    public IPSOLTVelocityProvider getClone() {
        return new IPSOLTVelocityProvider(this);
    }
    ...
    public Vector get(Particle particle) {
        ...
        Vector cognitiveComponent = Vector.copyOf(localGuide).subtract(position).multiply(cp(cognitiveAcceleration)).multiply(cp(inertiaWeight)).multiply(random());
