package cilib.math.truncatednormal;
...
public class TruncatedNormal extends AbstractRealDistribution {
    ...
    protected double cdf_a;
    protected double cdf_b;
    protected double Z;
    ...
    public TruncatedNormal(RandomGenerator rng, double mean, double sd, double lb, double ub) {
        ...
        mu = mean;
        sigma = sd;
        cdf_a = NormalDistribution.cdf01((lb - mu)/sigma);
        cdf_b = NormalDistribution.cdf01((ub - mu)/sigma);
        Z = cdf_b - cdf_a;
    ...
    public void updateParameters(double mean, double sd){
        mu = mean;
        sigma = sd;
        cdf_a = NormalDistribution.cdf01((aa - mu)/sigma);
        cdf_b = NormalDistribution.cdf01((bb - mu)/sigma);
        Z = cdf_b - cdf_a;
    ...
    public double density(double x) {
        ...
        return NormalDistribution.density01((x - mu)/sigma) / (sigma * Z);
    }
    ...
    public double cumulativeProbability(double x) {
        ...
        double u = NormalDistribution.cdf01((x - mu)/sigma);
        return (u - cdf_a) / Z;
    }
    ...
    public double inverseCumulativeProbability(double p) throws OutOfRangeException {
        ...
        return mu + sigma * NormalDistribution.inverseF01(val);
    }

    @Override
    public double getNumericalMean() {
        double phi_a = NormalDistribution.density01(alpha);
        double phi_b = NormalDistribution.density01(beta);
    ...
    public double sample(Bounds bounds){
        double sample = sample();

        return bounds.getRange() * ((sample + 1) / 2) + bounds.getLowerBound();
    }
