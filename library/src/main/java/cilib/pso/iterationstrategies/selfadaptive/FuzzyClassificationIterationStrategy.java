...
public class FuzzyClassificationIterationStrategy extends AbstractIterationStrategy<PSO>{
    ...
    public void performIteration(PSO algorithm) {
        ...
        //TODO: is the 'globally best particle' the gbest or the iteration best?
    ...
    protected void changeState(double memExplore, double memExploit, double memConv, double memJump){
        ...
        switch(state){
            case EXPLORATION:
                if(memExploit > 0){         //favor change to exploitation
                    state = state.EXPLOITATION;
                }
                else if(memExplore > 0){   //resist change
                    state = State.EXPLORATION;
                }
                ...
                break;
            case EXPLOITATION:
                if(memConv > 0){         //favor change to convergence
                    state = state.CONVERGENCE;
                }
                else if(memExploit > 0){   //resist change
                    state = State.EXPLOITATION;
                }
                ...
                break;
            case CONVERGENCE:
                if(memJump > 0){         //favor change to jumping out
                    state = state.JUMPINGOUT;
                }
                else if(memConv > 0){   //resist change
                    state = State.CONVERGENCE;
                }
                ...
                break;
            case JUMPINGOUT:
                if(memExplore > 0){         //favor change to convergence
                    state = state.EXPLORATION;
                }
                else if(memJump > 0){       //resist change
                    state = State.JUMPINGOUT;
                }
                else if(memExplore > memConv){
                    state = State.EXPLORATION;
                }
                else{
                    state = State.CONVERGENCE;
